...
def get_move_legend_above_ax(plot_ax, bbox_to_anchor = None, **kwargs):
    ...
    #move to location
    plot_ax.legend(hand, labs, bbox_to_anchor=bbox_to_anchor,loc = 'lower center', ncols = 4, frameon = False, **legend_defaults)
...
def ax_add_stage_names_and_outline_points(ax_obj):
    ...
    set_pointplot_edgecolor(ax_obj, edge_color = 'black', linewidth = marker_edge_width)# replaces: # for s in sc.collections:# s.set_edgecolor("black"), s.set_linewidth(marker_edge_width)
    sec, sec2, is_dict = set_stage_xticks_2_layer(ax_obj, phase_list_IA_RS_chrono)
